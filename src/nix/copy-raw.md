...
* Copy Firefox from the local store to a binary cache in `/tmp/cache`:
  ...
  ```console
  # nix copy-raw --to file:///tmp/cache $(type -p firefox)
  ```
...
* Copy the entire current NixOS system closure to another machine via SSH:
  ...
  ```console
  # nix copy-raw -s --to ssh://server /run/current-system
  ```
...
* Copy a closure from another machine via SSH:
  ...
  ```console
  # nix copy-raw --from ssh://server /nix/store/a6cnl93nk1wxnq84brbbwr6hxw9gp2w9-blender-2.79-rc2
  ```

* Copy Hello to a binary cache in an Amazon S3 bucket:

  ```console
  # nix copy-raw --to s3://my-bucket?region=eu-west-1 nixpkgs#hello
  ```

  or to an S3-compatible storage system:

  ```console
  # nix copy-raw --to s3://my-bucket?region=eu-west-1&endpoint=example.com nixpkgs#hello
  ```
...
* Copy a closure from `/nix/store` to the chroot store `/tmp/nix/nix/store`:
  ...
  ```console
  # nix copy-raw --to /tmp/nix nixpkgs#hello --no-check-sigs
  ```

# Description

`nix copy-raw` copies store path closures between two Nix stores. The
